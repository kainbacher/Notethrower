...
class MCAPI {
    ...
    */
    ...
    function callServer($method, $params) {
        ...
        ob_start();
        if ($this->secure){
            $sock = fsockopen("ssl://".$host, 443, $errno, $errstr, 60); // hjonas, changed timeout from 30 to 60
        } else {
            $sock = fsockopen($host, 80, $errno, $errstr, 60); // hjonas, changed timeout from 30 to 60
        }
        if(!$sock) {
